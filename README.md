...
## tokensSimplified.json

The `tokensSimplified.json` file is a streamlined version of the tokens list that includes only tokens used in bills (bonds). This file is automatically generated during the build process and offers several advantages:

### Key Features

- **Bills-only tokens**: Contains only tokens that are used as `lpToken` or `earnToken` in bills
- **Chain-specific filtering**: For each token, only includes addresses and decimals for chains where the token is actually used in bills
- **Smaller file size**: Reduced from 637 tokens to ~401 tokens, with optimized chain data
- **Consistent key format**: Uses the exact same property names as defined in `tokens.ts`

### Example

Original `tokens.json` might have USDC with addresses for 18 chains, but `tokensSimplified.json` only includes the 13 chains where USDC is actually used in bills:
...
```json
{
  "usdc": {
    ...
    "address": {
      ...
      // ... only chains used in bills
    },
    "decimals": {
      "1": 6,
      "56": 18,
      "137": 6
      // ... corresponding decimals for used chains only
    },
    "active": true
  }
}
...
The file is automatically generated when running `yarn build` and can be found at `config/tokensSimplified.json`. Use this file when you need a lightweight token list that focuses specifically on tokens used in the bills ecosystem.
